...
bool CvTechInfo::isCivSettled() const {
...
bool CvTechInfo::read(CvXMLLoadUtility* pXML) {
	...
	pXML->GetChildXmlValByName(&m_bCivSettled, "bCivSettled");
...
int CvPromotionInfo::getSpyStealTreasuryChange() const {
	return m_iSpyStealTreasuryChange;
}
...
bool CvPromotionInfo::read(CvXMLLoadUtility* pXML) {
	...
	pXML->GetChildXmlValByName(&m_iSpyStealTreasuryChange, "iSpyStealTreasuryChange");
...
bool CvCivicInfo::read(CvXMLLoadUtility* pXML) {
	...
	pXML->GetChildXmlValByName(&m_iDistantUnitSupplyCostModifier, "iDistantUnitSupplyCostModifier");
...
bool CvBuildingInfo::isApplyAllFreePromotionsOnMove() const {
	return m_bApplyAllFreePromotionsOnMove;
}
...
int CvBuildingInfo::getTechYieldChange(int i, int j) const {
	...
	FAssertMsg(j < NUM_YIELD_TYPES, "Index out of bounds");
	FAssertMsg(j > -1, "Index out of bounds");
	return m_ppaiTechYieldChange ? m_ppaiTechYieldChange[i][j] : -1;
}
...
bool CvBuildingInfo::isAnyTechYieldChange() const {
	return m_bAnyTechYieldChange;
}
...
bool CvBuildingInfo::isPrereqAndTerrain(TerrainTypes eTerrain) const {
	return (std::find(m_viPrereqAndTerrains.begin(), m_viPrereqAndTerrains.end(), eTerrain) != m_viPrereqAndTerrains.end());
}

bool CvBuildingInfo::isPrereqOrTerrain(TerrainTypes eTerrain) const {
	return (std::find(m_viPrereqOrTerrains.begin(), m_viPrereqOrTerrains.end(), eTerrain) != m_viPrereqOrTerrains.end());
}
...
bool CvBuildingInfo::isPrereqVicinityImprovement(ImprovementTypes eImprovement) const {
	return (std::find(m_viPrereqVicinityImprovements.begin(), m_viPrereqVicinityImprovements.end(), eImprovement) != m_viPrereqVicinityImprovements.end());
}
...
bool CvBuildingInfo::isPrereqVicinityFeature(FeatureTypes eFeature) const {
	return (std::find(m_viPrereqVicinityFeatures.begin(), m_viPrereqVicinityFeatures.end(), eFeature) != m_viPrereqVicinityFeatures.end());
}
...
bool CvBuildingInfo::isPrereqVicinityAndBonus(BonusTypes eBonus) const {
	return (std::find(m_viPrereqVicinityAndBonus.begin(), m_viPrereqVicinityAndBonus.end(), eBonus) != m_viPrereqVicinityAndBonus.end());
}
...
bool CvBuildingInfo::isPrereqAndBuildingClass(int iBuildingClass) const {
	return (std::find(m_viPrereqAndBuildingClasses.begin(), m_viPrereqAndBuildingClasses.end(), (BuildingClassTypes)iBuildingClass) != m_viPrereqAndBuildingClasses.end());
}
...
bool CvBuildingInfo::isPrereqOrBuildingClass(int iBuildingClass) const {
	return (std::find(m_viPrereqOrBuildingClasses.begin(), m_viPrereqOrBuildingClasses.end(), (BuildingClassTypes)iBuildingClass) != m_viPrereqOrBuildingClasses.end());
}
...
bool CvBuildingInfo::isPrereqNotBuildingClass(BuildingClassTypes eBuildingClass) const {
	return (std::find(m_viPrereqNotBuildingClasses.begin(), m_viPrereqNotBuildingClasses.end(), eBuildingClass) != m_viPrereqNotBuildingClasses.end());
}

int CvBuildingInfo::getMinPopulation() const {
	return m_iMinPopulation;
}

CultureLevelTypes CvBuildingInfo::getMinCultureLevel() const {
	return m_eMinCultureLevel;
}
...
void CvBuildingInfo::read(FDataStreamBase* stream) {
	...
	m_bAnyTechYieldChange = false;
	for (int i = 0; !m_bAnyTechYieldChange && i < GC.getNumTechInfos(); i++) {
		for (int j = 0; j < NUM_YIELD_TYPES; j++) {
			if (m_ppaiTechYieldChange[i][j] != 0) {
				m_bAnyTechYieldChange = true;
				break;
			}
		}
	}
...
bool CvBuildingInfo::read(CvXMLLoadUtility* pXML) {
	...
	pXML->SetVectorInfo(m_viPrereqVicinityFeatures, "PrereqVicinityFeatures");
	pXML->GetChildXmlValByName(&m_iMinPopulation, "iMinPopulation");
	pXML->GetChildXmlValByName(szTextVal, "MinCultureLevel");
	m_eMinCultureLevel = (CultureLevelTypes)pXML->FindInInfoClass(szTextVal);
	pXML->GetChildXmlValByName(&m_bPrereqPower, "bPrereqPower");
	...
	m_bAnyTechYieldChange = pXML->SetListPairInfoArray(&m_ppaiTechYieldChange, "TechYieldChanges", GC.getNumTechInfos(), NUM_YIELD_TYPES);
...
bool CvYieldInfo::read(CvXMLLoadUtility* pXML) {
	...
	pXML->GetChildXmlValByName(&m_iAIWeightPercent, "iAIWeightPercent", 100);
...
CvTraitInfo::~CvTraitInfo() {
	...
	if (m_ppaiBuildingClassYieldChange != NULL) {
		for (int i = 0; i < GC.getNumBuildingClassInfos(); i++) {
			SAFE_DELETE_ARRAY(m_ppaiBuildingClassYieldChange[i]);
		}
...
int CvTraitInfo::getBuildingClassYieldChange(int i, int j) const {
	...
	FAssertMsg(j < NUM_YIELD_TYPES, "Index out of bounds");
	FAssertMsg(j > -1, "Index out of bounds");
	return m_ppaiBuildingClassYieldChange ? m_ppaiBuildingClassYieldChange[i][j] : -1;
}

int* CvTraitInfo::getBuildingClassYieldChangeArray(int i) const {
	FAssertMsg(i < GC.getNumBuildingClassInfos(), "Index out of bounds");
	FAssertMsg(i > -1, "Index out of bounds");
	return m_ppaiBuildingClassYieldChange[i];
}

bool CvTraitInfo::isAnyBuildingClassYieldChange() const {
	return m_bAnyBuildingClassYieldChange;
}

bool CvTraitInfo::isAnyBuildingClassYieldChange(int i) const {
	FAssertMsg(i < GC.getNumBuildingClassInfos(), "Index out of bounds");
	FAssertMsg(i > -1, "Index out of bounds");
	bool bChanges = false;
	for (int j = 0; j < NUM_YIELD_TYPES; j++) {
		if (m_ppaiBuildingClassYieldChange[i][j] != 0) {
			bChanges = true;
			break;
		}
	}
...
bool CvTraitInfo::read(CvXMLLoadUtility* pXML) {
	...
	m_bAnyBuildingClassYieldChange = pXML->SetListPairInfoArray(&m_ppaiBuildingClassYieldChange, "BuildingClassYieldChanges", GC.getNumBuildingClassInfos(), NUM_YIELD_TYPES);
...
bool CvEraInfo::read(CvXMLLoadUtility* pXML) {
	...
	pXML->SetList(&m_piNaturalYieldLimit, "NaturalYieldLimits", NUM_YIELD_TYPES, -1);
...
bool CvEventTriggerInfo::read(CvXMLLoadUtility* pXML) {
	...
	pXML->SetVectorInfo(m_aiUnitsRequired, "UnitsRequired");
	pXML->SetVectorInfo(m_aiBuildingsRequired, "BuildingsRequired");
	pXML->SetVectorInfo(m_aiPrereqOrTechs, "OrPreReqs");
	pXML->SetVectorInfo(m_aiPrereqAndTechs, "AndPreReqs");
	...
	pXML->SetVectorInfo(m_aiObsoleteTechs, "ObsoleteTechs");
	pXML->SetVectorInfo(m_aiEvents, "Events");
	pXML->SetVectorInfo(m_aiPrereqEvents, "PrereqEvents");
	pXML->SetVectorInfo(m_aiFeaturesRequired, "FeaturesRequired");
	pXML->SetVectorInfo(m_aiTerrainsRequired, "TerrainsRequired");
	pXML->SetVectorInfo(m_aiImprovementsRequired, "ImprovementsRequired");
	pXML->SetVectorInfo(m_aiBonusesRequired, "BonusesRequired");
	pXML->SetVectorInfo(m_aiRoutesRequired, "RoutesRequired");
	pXML->SetVectorInfo(m_aiReligionsRequired, "ReligionsRequired");
	pXML->SetVectorInfo(m_aiCorporationsRequired, "CorporationsRequired");
