											...
											if (GC.getGameINLINE().getSorenRandNum(GC.getLeaderHeadInfo(getPersonalityType()).getContactRand(CONTACT_PEACE_TREATY), "AI Diplo Peace Treaty") == 0) {
												...
												if (canTradeItem(((PlayerTypes)iI), item, true) && GET_PLAYER((PlayerTypes)iI).canTradeItem(getID(), item, true)) {
													...
													if (iTheirValue > iOurValue) {
														...
														int iGold = std::min((iTheirValue - iOurValue), GET_PLAYER((PlayerTypes)iI).AI_maxGoldTrade(getID()));
														...
														for (int iJ = 0; iJ < GC.getNumTechInfos(); iJ++) {
															...
															if (canTradeItem(((PlayerTypes)iI), item, true)) {
																if (GET_TEAM(GET_PLAYER((PlayerTypes)iI).getTeam()).AI_techTradeVal((TechTypes)iJ, getTeam()) <= (iOurValue - iTheirValue)) {
																	int iValue = (1 + GC.getGameINLINE().getSorenRandNum(10000, "AI Peace Trading (Tech #2)"));
														...
														int iGold = std::min((iOurValue - iTheirValue), AI_maxGoldTrade((PlayerTypes)iI));
														...
														for (CvCity* pLoopCity = firstCity(&iLoop); pLoopCity != NULL; pLoopCity = nextCity(&iLoop)) {
															...
															if (canTradeItem(((PlayerTypes)iI), item, true)) {
																if (GET_PLAYER((PlayerTypes)iI).AI_cityTradeVal(pLoopCity) <= (iOurValue - iTheirValue)) {
																	int iValue = pLoopCity->plot()->calculateCulturePercent((PlayerTypes)iI);
...
void CvPlayerAI::AI_conquerCity(CvCity* pCity) {
	...
	// If we cant capture cities all we can do is raze it
	if (!canCaptureCities()) {
		bRaze = true;
	} else {
...
bool CvPlayerAI::AI_captureUnit(UnitTypes eUnit, CvPlot* pPlot) const {
	...
	CvCity* pNearestCity = GC.getMapINLINE().findCity(pPlot->getX_INLINE(), pPlot->getY_INLINE(), NO_PLAYER, getTeam());
...
TechTypes CvPlayerAI::AI_bestTech(int iMaxPathLength, bool bFreeTech, bool bAsync, TechTypes eIgnoreTech, AdvisorTypes eIgnoreAdvisor) const {
	...
	for (BonusTypes eBonus = (BonusTypes)0; eBonus < GC.getNumBonusInfos(); eBonus = (BonusTypes)(eBonus + 1)) {
		...
		BonusClassTypes eBonusClass = (BonusClassTypes)GC.getBonusInfo(eBonus).getBonusClassType();
		if (eRevealTech != NO_TECH) {
			...
			} else if (countOwnedBonuses(eBonus) > 0) {
				viBonusClassHave[eBonusClass]++;
			}
		}
	}
	...
	for (int iDepth = 0; iDepth < iMaxPathLength; ++iDepth) {
		...
		for (TechTypes eTech = (TechTypes)0; eTech < GC.getNumTechInfos(); eTech = (TechTypes)(eTech + 1)) {
			...
			if (kTech.getEra() > (getCurrentEra() + 1))
			...
			// Check "and" prereqs
			for (int p = 0; p < kTech.getNumPrereqAndTechs(); ++p) {
...
// Note: many of the values used in this function are arbitrary; but I've adjusted them all to get closer to having a common scale.
...
int CvPlayerAI::AI_techValue(TechTypes eTech, int iPathLength, bool bFreeTech, bool bAsync, const std::vector<int>& viBonusClassRevealed, const std::vector<int>& viBonusClassUnrevealed, const std::vector<int>& viBonusClassHave) const {
	...
	if (kTechInfo.isMapVisible()) {
		...
		// Note, the world is usually thoroughly explored by the time of satilites. So this is low value.
		// If we wanted to evaluate this properly, we'd need to calculate at how much of the world is still unexplored.
	}

	// Value taken from civic calculation, this is roughly a number between 65 and 167
	int iWarmongerFactor = 25000 / std::max(100, (100 + GC.getLeaderHeadInfo(getPersonalityType()).getMaxWarRand()));
	if (kTechInfo.isUnitTerritoryUnbound() && !isUnitTerritoryUnbound()) {
		int iTemp = getNumUnits() * iWarmongerFactor;
		iTemp /= 100;
		iValue += iTemp;
	}
	if (kTechInfo.isUnitRangeUnbound() && !isUnitRangeUnbound()) {
		int iTemp = getNumMilitaryUnits() * iWarmongerFactor;
		iTemp /= 100;
		iValue += iTemp;
	}
	if (kTechInfo.getUnitRangeChange() != 0 && !isUnitRangeUnbound()) {
		int iTemp = kTechInfo.getUnitRangeChange() * getNumMilitaryUnits() * iWarmongerFactor;
		iTemp /= 100;
		iValue += iTemp;
	}
	...
	if (kTechInfo.isMapTrading() && !kTeam.isMapTrading()) // K-Mod
	{
		...
		for (TeamTypes eLoopTeam = (TeamTypes)0; eLoopTeam < MAX_CIV_TEAMS; eLoopTeam = (TeamTypes)(eLoopTeam + 1)) {
			if (eLoopTeam == getTeam() || !kTeam.isHasMet(eLoopTeam))
				continue;
			const CvTeamAI& kLoopTeam = GET_TEAM(eLoopTeam);
			if (!kLoopTeam.isMapTrading()) {
				if (kLoopTeam.AI_mapTrade(getTeam()) == NO_DENIAL && kTeam.AI_mapTrade(eLoopTeam) == NO_DENIAL)
	...
	if (kTechInfo.isTechTrading() && !GC.getGameINLINE().isOption(GAMEOPTION_NO_TECH_TRADING) && !kTeam.isTechTrading()) // K-Mod
	{
		...
		for (TeamTypes eLoopTeam = (TeamTypes)0; eLoopTeam < MAX_CIV_TEAMS; eLoopTeam = (TeamTypes)(eLoopTeam + 1)) {
			...
			if (!kLoopTeam.isTechTrading()) {
				if (kLoopTeam.AI_techTrade(NO_TECH, getTeam()) == NO_DENIAL && kTeam.AI_techTrade(NO_TECH, eLoopTeam) == NO_DENIAL)
					iNewTrade += kLoopTeam.getAliveCount();
			} else
				iExistingTrade += kLoopTeam.getAliveCount();
		}
	...
	if (kTechInfo.isEmbassyTrading()) {
		if (iHasMetCount > 0) {
			iValue += 8 * iCityCount;
		}
	}

	if (kTechInfo.isLimitedBordersTrading()) {
	...
	if (kTechInfo.isDefensivePactTrading() && !kTeam.isDefensivePactTrading()) {
		...
		for (TeamTypes eLoopTeam = (TeamTypes)0; eLoopTeam < MAX_CIV_TEAMS; eLoopTeam = (TeamTypes)(eLoopTeam + 1)) {
			...
			if (!kLoopTeam.isDefensivePactTrading()) {
				if (kLoopTeam.AI_defensivePactTrade(getTeam()) == NO_DENIAL && kTeam.AI_defensivePactTrade(eLoopTeam) == NO_DENIAL)
					iNewTrade += kLoopTeam.getAliveCount();
			} else
				iExistingTrade += kLoopTeam.getAliveCount();
		}
	...
	for (RouteTypes eRoute = (RouteTypes)0; eRoute < GC.getNumRouteInfos(); eRoute = (RouteTypes)(eRoute + 1)) {
		...
		if (GC.getRouteInfo(eRoute).getTechMovementChange(eTech)) {
			int iTemp = 4 * (std::max(iCityCount, iCityTarget) - iCoastalCities / 2);
			if (bCapitalAlone)
				iTemp = iTemp * 2 / 3;
			if (kTeam.getAnyWarPlanCount(true))
				iTemp *= 2;
			iValue -= GC.getRouteInfo(eRoute).getTechMovementChange(eTech) * iTemp;
		}
	}
	...
	for (CommerceTypes eCommerce = (CommerceTypes)0; eCommerce < NUM_COMMERCE_TYPES; eCommerce = (CommerceTypes)(eCommerce + 1)) {
		...
		if (bSpecialistCommerce) {
			iCommerceValue += 4 * AI_averageCommerceMultiplier(eCommerce) * (kTechInfo.getSpecialistExtraCommerce(eCommerce) * std::max((getTotalPopulation() + 12 * iTotalBonusSpecialists) / 12, iTotalCurrentSpecialists));
		}

		// Commerce multipliers. (using iCommerceValue from the specialists bonuses to calculate compound effect).
		iCommerceValue += kTechInfo.getCommerceModifier(eCommerce) * (4 * getCommerceRate(eCommerce) + iCommerceValue) / AI_averageCommerceMultiplier(eCommerce);
		...
		if (iCommerceValue) {
			iCommerceValue *= AI_commerceWeight(eCommerce);
			iCommerceValue /= 100;
	...
	for (TerrainTypes eTerrain = (TerrainTypes)0; eTerrain < GC.getNumTerrainInfos(); eTerrain = (TerrainTypes)(eTerrain + 1)) {
		if (kTechInfo.isTerrainTrade(eTerrain)) {
	...
	for (ImprovementTypes eImprovement = (ImprovementTypes)0; eImprovement < GC.getNumImprovementInfos(); eImprovement = (ImprovementTypes)(eImprovement + 1)) {
		for (YieldTypes eYield = (YieldTypes)0; eYield < NUM_YIELD_TYPES; eYield = (YieldTypes)(eYield + 1)) {
			...
			iTempValue += GC.getImprovementInfo(eImprovement).getTechYieldChanges(eTech, eYield) * std::max(getImprovementCount(eImprovement), 3 * getNumCities() / 2) * 4;
	...
	for (BuildTypes eLoopBuild = (BuildTypes)0; eLoopBuild < GC.getNumBuildInfos(); eLoopBuild = (BuildTypes)(eLoopBuild + 1)) {
		if (GC.getBuildInfo(eLoopBuild).getTechPrereq() == eTech) {
			...
			if (eBuildImprovement == NO_IMPROVEMENT) {
				...
				for (TerrainTypes eTerrain = (TerrainTypes)0; eTerrain < GC.getNumTerrainInfos(); eTerrain = (TerrainTypes)(eTerrain + 1)) {
					iAccessibility += (kBuildImprovement.getTerrainMakesValid(eTerrain) ? 75 : 0);
				}

				for (FeatureTypes eFeature = (FeatureTypes)0; eFeature < GC.getNumFeatureInfos(); eFeature = (FeatureTypes)(eFeature + 1)) {
					if (kBuildImprovement.getFeatureMakesValid(eFeature)) {
						iAccessibility += 75;
						int iTempValue = 0;
						for (YieldTypes eYield = (YieldTypes)0; eYield < NUM_YIELD_TYPES; eYield = (YieldTypes)(eYield + 1)) {
							iTempValue += GC.getFeatureInfo(eFeature).getYieldChange(eYield) * AI_yieldWeight(eYield) * AI_averageYieldMultiplier(eYield) / 100;
						}
				...
				if (iAccessibility > 0) {
					...
					for (YieldTypes eYield = (YieldTypes)0; eYield < NUM_YIELD_TYPES; eYield = (YieldTypes)(eYield + 1)) {
						...
						iTempValue += kBuildImprovement.getYieldChange(eYield) * 100;
						iTempValue += kBuildImprovement.getRiverSideYieldChange(eYield) * 50;
						iTempValue += kBuildImprovement.getHillsYieldChange(eYield) * 75;
						iTempValue += kBuildImprovement.getIrrigatedYieldChange(eYield) * 80;

						iTempValue += kFinalImprovement.getYieldChange(eYield) * 100;
						iTempValue += kFinalImprovement.getRiverSideYieldChange(eYield) * 50;
						iTempValue += kFinalImprovement.getHillsYieldChange(eYield) * 75;
						iTempValue += kFinalImprovement.getIrrigatedYieldChange(eYield) * 80;
				...
				for (BonusTypes eBonus = (BonusTypes)0; eBonus < GC.getNumBonusInfos(); eBonus = (BonusTypes)(eBonus + 1)) {
					...
					if (!kFinalImprovement.isImprovementBonusMakesValid(eBonus) && !kFinalImprovement.isImprovementBonusTrade(eBonus))
						continue;

					bool bRevealed = kTeam.isBonusRevealed(eBonus);

					int iNumBonuses = bRevealed
						? countOwnedBonuses(eBonus) // actual count
					...
					if (iNumBonuses > 0 && (bRevealed || kBonusInfo.getTechReveal() == eTech)) {
						...
						if ((kTeam.isHasTech(eConnectTech) || eConnectTech == eTech) && !kTeam.isBonusObsolete(eBonus) && kBonusInfo.getTechObsolete() != eTech) {
							// note: this is in addition to the getTechCityTrade evaluation lower in this function.
							iBonusValue += AI_bonusVal(eBonus, 1, true) * iCityCount;
							if (bRevealed)
						...
						for (YieldTypes eYield = (YieldTypes)0; eYield < NUM_YIELD_TYPES; eYield = (YieldTypes)(eYield + 1)) {
							...
							iTempValue += (kFinalImprovement.getImprovementBonusYield(eBonus, eYield) * 100);
							iTempValue += (kFinalImprovement.getIrrigatedYieldChange(eYield) * 75);
			...
			if (eRoute != NO_ROUTE) {
				...
				for (YieldTypes eYield = (YieldTypes)0; eYield < NUM_YIELD_TYPES; eYield = (YieldTypes)(eYield + 1)) {
					int iTempValue = 0;
					iTempValue += getNumCities() * GC.getRouteInfo(eRoute).getYieldChange(eYield) * 32; // epic bonus for epic effect
					for (ImprovementTypes eImprovement = (ImprovementTypes)0; eImprovement < GC.getNumImprovementInfos(); eImprovement = (ImprovementTypes)(eImprovement + 1)) {
						iTempValue += GC.getImprovementInfo(eImprovement).getRouteYieldChanges(eRoute, eYield) * std::max(getImprovementCount(eImprovement), 3 * getNumCities() / 2) * 4;
					}
					iTempValue *= AI_averageYieldMultiplier(eYield);
					iTempValue /= 100;
	...
	for (FeatureTypes eFeature = (FeatureTypes)0; eFeature < GC.getNumFeatureInfos(); eFeature = (FeatureTypes)(eFeature + 1)) {
		...
		for (BuildTypes eBuild = (BuildTypes)0; eBuild < GC.getNumBuildInfos(); eBuild = (BuildTypes)(eBuild + 1)) {
			if (GC.getBuildInfo(eBuild).getFeatureTech(eFeature) == eTech) {
				bIsFeatureRemove = true;
				iChopProduction = GC.getBuildInfo(eBuild).getFeatureProduction(eBuild); // I'll assume it's the same for all builds
				break;
			}
		}
		...
		if (bIsFeatureRemove) {
			...
			if ((GC.getFeatureInfo(eFeature).getHealthPercent() < 0) || ((GC.getFeatureInfo(eFeature).getYieldChange(YIELD_FOOD) + GC.getFeatureInfo(eFeature).getYieldChange(YIELD_PRODUCTION) + GC.getFeatureInfo(eFeature).getYieldChange(YIELD_COMMERCE)) < 0)) {
				iChopValue += 8;
	...
	for (BonusTypes eBonus = (BonusTypes)0; eBonus < GC.getNumBonusInfos(); eBonus = (BonusTypes)(eBonus + 1)) {
		if (GC.getBonusInfo(eBonus).getTechReveal() == eTech) {
			...
			//iMultiplier is basically a desperation value it gets larger as the AI runs out of options
		...
		else if (GC.getBonusInfo(eBonus).getTechCityTrade() == eTech &&
			(kTeam.isHasTech((TechTypes)GC.getBonusInfo(eBonus).getTechReveal()) || kTeam.isForceRevealedBonus(eBonus))) {
	...
	for (ProcessTypes eProcess = (ProcessTypes)0; eProcess < GC.getNumProcessInfos(); eProcess = (ProcessTypes)(eProcess + 1)) {
		if (GC.getProcessInfo(eProcess).getTechPrereq() == eTech) {
			iValue += 28;

			for (CommerceTypes eCommerce = (CommerceTypes)0; eCommerce < NUM_COMMERCE_TYPES; eCommerce = (CommerceTypes)(eCommerce + 1)) {
				int iTempValue = (GC.getProcessInfo(eProcess).getProductionToCommerceModifier(eCommerce) * 4);
				...
				if (eCommerce == COMMERCE_GOLD || eCommerce == COMMERCE_RESEARCH) {
					bIsGoodProcess = true;
				} else if ((eCommerce == COMMERCE_CULTURE) && AI_isDoVictoryStrategy(AI_VICTORY_CULTURE1)) {
					iTempValue *= 2;
				}
	...
	if (bIsGoodProcess && bFinancialTrouble) {
		...
		for (ProcessTypes eProcess = (ProcessTypes)0; eProcess < GC.getNumProcessInfos(); eProcess = (ProcessTypes)(eProcess + 1)) {
			if (kTeam.isHasTech((TechTypes)GC.getProcessInfo(eProcess).getTechPrereq())) {
				bHaveGoodProcess = (GC.getProcessInfo(eProcess).getProductionToCommerceModifier(COMMERCE_GOLD) + GC.getProcessInfo(eProcess).getProductionToCommerceModifier(COMMERCE_RESEARCH)) > 0;
				if (bHaveGoodProcess) {
					break;
				}
			}
	...
	for (CivicTypes eLoopCivic = (CivicTypes)0; eLoopCivic < GC.getNumCivicInfos(); eLoopCivic = (CivicTypes)(eLoopCivic + 1)) {
		if (GC.getCivicInfo(eLoopCivic).getTechPrereq() == eTech && !canDoCivics(eLoopCivic)) {
			CivicTypes eCivic = getCivics((CivicOptionTypes)(GC.getCivicInfo(eLoopCivic).getCivicOptionType()));
	...
	if (iPathLength <= 2) {
		...
		for (BuildingTypes eBuilding = (BuildingTypes)0; eBuilding < GC.getNumBuildingInfos(); eBuilding = (BuildingTypes)(eBuilding + 1)) {
			...
			if (kBuildingInfo.getFoundsCorporation() != NO_CORPORATION) {
				if (isTechRequiredForBuilding(eTech, eBuilding)) {
					...
					corpHQs[kBuildingInfo.getFoundsCorporation()] = eBuilding;
				}
			}
		}
		...
		for (CorporationTypes eCorporation = (CorporationTypes)0; eCorporation < GC.getNumCorporationInfos(); eCorporation = (CorporationTypes)(eCorporation + 1)) {
			if (!GC.getGameINLINE().isCorporationFounded(eCorporation)) {
				...
				if (GC.getCorporationInfo(eCorporation).getTechPrereq() == eTech) {
				...
				} else if (corpHQs[eCorporation] != NO_BUILDING) {
					const CvBuildingInfo& kBuildingInfo = GC.getBuildingInfo(corpHQs[eCorporation]);
				...
				if (bCorpTech) {
					int iCorpValue = AI_corporationValue(eCorporation); // roughly 100x commerce from corp.
		...
		if (getTechFreeUnit(eTech, false) != NO_UNIT) {
			int iUnitValue = AI_getUnitClassWeight((UnitClassTypes)GC.getTechInfo(eTech).getFirstFreeUnitClass());
			iValue += iUnitValue / 2;
			iValue += bAsync ? GC.getASyncRand().get(iUnitValue, "AI Research Free Unit ASYNC") : GC.getGameINLINE().getSorenRandNum(iUnitValue, "AI Research Free Unit");
			iRandomMax += iUnitValue;
		}
		...
		if (bFirst) {
			...
			for (ReligionTypes eReligion = (ReligionTypes)0; eReligion < GC.getNumReligionInfos(); eReligion = (ReligionTypes)(eReligion + 1)) {
				TechTypes eReligionTech = (TechTypes)GC.getReligionInfo(eReligion).getTechPrereq();
			...
			if (iReligionValue > 0) {
				...
				if ((countTotalHasReligion() == 0) && (iPotentialReligions == 0)) {
					...
					for (PlayerTypes eLoopPlayer = (PlayerTypes)0; !bNeighbouringReligions && eLoopPlayer < MAX_CIV_PLAYERS; eLoopPlayer = (PlayerTypes)(eLoopPlayer + 1)) {
						const CvPlayer& kLoopPlayer = GET_PLAYER(eLoopPlayer);
						if (kLoopPlayer.isAlive() && kTeam.isHasMet(kLoopPlayer.getTeam()) && kLoopPlayer.getStateReligion() != NO_RELIGION && kTeam.AI_hasSharedPrimaryArea(kLoopPlayer.getTeam())) {
					...
					if (iAvailableReligions <= 4 || AI_getFlavorValue(FLAVOR_RELIGION) > 0) {
						iReligionValue *= 2;
						iReligionValue += 56 + std::max(0, 6 - iAvailableReligions) * 100;
					} else {
			...
			if (kTechInfo.getFirstFreeTechs() > 0) {
				int iBase = 100; // previous base value was 1600 - which we'll now calculate based on costs of currently researchable tech
	...
	if (!isHuman()) {
		...
		for (FlavorTypes eFlavor = (FlavorTypes)0; eFlavor < GC.getNumFlavorTypes(); eFlavor = (FlavorTypes)(eFlavor + 1)) {
			iFlavorValue += AI_getFlavorValue(eFlavor) * kTechInfo.getFlavorValue(eFlavor) * 5;
		}
	...
	//Tech Groundbreaker								
	if (!GC.getGameINLINE().isOption(GAMEOPTION_NO_TECH_TRADING) && !isBarbarian() && iPathLength <= 1) {
		if (kTechInfo.isTechTrading() || kTeam.isTechTrading()) {
			...
			if (AI_getStrategyRand(GC.getGameINLINE().getGameTurn()) % std::max(1, GC.getLeaderHeadInfo(getPersonalityType()).getContactRand(CONTACT_TRADE_TECH)) == 0) {
				...
				for (TeamTypes eLoopTeam = (TeamTypes)0; eLoopTeam < MAX_CIV_TEAMS; eLoopTeam = (TeamTypes)(eLoopTeam + 1)) {
					const CvTeamAI& kLoopTeam = GET_TEAM(eLoopTeam);
					if (eLoopTeam != getTeam() && kLoopTeam.isAlive() && kTeam.isHasMet(eLoopTeam)) {
						if (kLoopTeam.isHasTech(eTech) || (canSeeResearch(kLoopTeam.getLeaderID()) && GET_PLAYER(kLoopTeam.getLeaderID()).getCurrentResearch() == eTech))
							iAlreadyKnown++;
						else if (!kTeam.isAtWar(eLoopTeam) && kLoopTeam.AI_techTrade(NO_TECH, getTeam()) == NO_DENIAL && kTeam.AI_techTrade(NO_TECH, eLoopTeam) == NO_DENIAL)
							iPotentialTrade++;
					}
				}
	...
	// K-Mod - Use a random _factor_ at the end.
...
int CvPlayerAI::AI_techBuildingValue(TechTypes eTech, bool bConstCache, bool& bEnablesWonder) const {
	...
	if (getNumCities() == 1 && getCurrentEra() == GC.getGameINLINE().getStartEra())
		iScale = iScale /= 2; // I expect we'll want to be building mostly units until we get a second city.
		...
		if (kLoopUnit.getPrereqAndTech(0) == eTech || kTeam.isHasTech((TechTypes)kLoopUnit.getPrereqAndTech(0)) || canResearch((TechTypes)kLoopUnit.getPrereqAndTech(0))) {
			...
			for (UnitAITypes eAI = (UnitAITypes)0; eAI < NUM_UNITAI_TYPES; eAI = (UnitAITypes)(eAI + 1)) {
				...
				case UNITAI_SLAVER:
					...
					iUtilityValue = std::max(iUtilityValue, (bCapitalAlone ? 2 : 4) * iWeight);
			...
			if (kLoopUnit.getUnitAIType(UNITAI_ASSAULT_SEA) && iCoastalCities > 0) {
				...
				if (AI_bestAreaUnitAIValue(UNITAI_ASSAULT_SEA, NULL, &eExistingUnit) == 0) {
					...
					iAssaultValue += 500 * std::max(0, AI_unitImpassableCount(eLoopUnit) - AI_unitImpassableCount(eExistingUnit));
					...
					iAssaultValue += (200 * (iNewCapacity - iOldCapacity)) / std::max(1, iOldCapacity);
				}
...
void CvPlayerAI::AI_chooseResearch() {
	...
	if (getCurrentResearch() == NO_TECH) {
		...
		if (eBestTech == NO_TECH) {
			int iResearchDepth = (isHuman() || isBarbarian() || AI_isDoVictoryStrategy(AI_VICTORY_CULTURE3) || AI_isDoStrategy(AI_STRATEGY_ESPIONAGE_ECONOMY)) ? 1 : 3;
...
void CvPlayerAI::AI_updateAttitudeCache(PlayerTypes ePlayer) {
	...
	for (MemoryTypes eMemory = (MemoryTypes)0; eMemory < NUM_MEMORY_TYPES; eMemory = (MemoryTypes)(eMemory + 1)) {
		iAttitude += AI_getMemoryAttitude(ePlayer, eMemory);
	}
...
int CvPlayerAI::AI_dealVal(PlayerTypes ePlayer, const CLinkList<TradeData>* pList, bool bIgnoreAnnual, int iChange) const {
	...
	if (atWar(getTeam(), eOtherTeam)) {
		iValue += kOurTeam.AI_endWarVal(eOtherTeam);
	}
	...
	for (CLLNode<TradeData>* pNode = pList->head(); pNode; pNode = pList->next(pNode)) {
		...
		case TRADE_TECHNOLOGIES:
			iValue += kOurTeam.AI_techTradeVal((TechTypes)(pNode->m_data.m_iData), eOtherTeam);
			break;
		...
		case TRADE_GOLD_PER_TURN:
			...
			break;
		case TRADE_MAPS:
			iValue += kOurTeam.AI_mapTradeVal(eOtherTeam);
			break;
		case TRADE_SURRENDER:
			if (!bIgnoreAnnual) {
				iValue += kOurTeam.AI_surrenderTradeVal(eOtherTeam);
			}
			break;
		case TRADE_VASSAL:
			if (!bIgnoreAnnual) {
				iValue += kOurTeam.AI_vassalTradeVal(eOtherTeam);
			}
			break;
		case TRADE_OPEN_BORDERS:
			iValue += kOurTeam.AI_openBordersTradeVal(eOtherTeam);
			break;
		case TRADE_DEFENSIVE_PACT:
			iValue += kOurTeam.AI_defensivePactTradeVal(eOtherTeam);
			break;
		case TRADE_PEACE:
			iValue += kOurTeam.AI_makePeaceTradeVal(((TeamTypes)(pNode->m_data.m_iData)), eOtherTeam);
			break;
		case TRADE_WAR:
			iValue += kOurTeam.AI_declareWarTradeVal(((TeamTypes)(pNode->m_data.m_iData)), eOtherTeam);
			break;
		case TRADE_EMBARGO:
		...
		case TRADE_RELIGION:
			...
			break;
		case TRADE_EMBASSY:
			iValue += kOurTeam.AI_embassyTradeVal(eOtherTeam);
			break;
...
bool CvPlayerAI::AI_counterPropose(PlayerTypes ePlayer, const CLinkList<TradeData>* pTheirList, const CLinkList<TradeData>* pOurList, CLinkList<TradeData>* pTheirInventory, CLinkList<TradeData>* pOurInventory, CLinkList<TradeData>* pTheirCounter, CLinkList<TradeData>* pOurCounter) const {
	...
	const CvTeamAI& kOurTeam = GET_TEAM(getTeam());
	const CvTeamAI& kTheirTeam = GET_TEAM(kTheirPlayer.getTeam());
	...
	int iValueForThem = kTheirPlayer.AI_dealVal(getID(), pOurList);
	...
	if (atWar(getTeam(), kTheirPlayer.getTeam())) {
		...
		if (!bEndWar) {
			...
			if (canTradeItem(ePlayer, item, true)) {
				...
				bEndWar = true;
			} else if (kTheirPlayer.canTradeItem(getID(), item, true)) {
	...
	if (kTheirPlayer.isHuman()) {
		...
		if (isHuman()) {
			...
			bDeal = 4 * iValueForUs >= iValueForThem
				&& 100 * iValueForThem >= kTheirPlayer.AI_tradeAcceptabilityThreshold(getID()) * iValueForUs;
		} else {
...
int CvPlayerAI::AI_bonusTradeVal(BonusTypes eBonus, PlayerTypes ePlayer, int iChange) const {
	...
	int iValue = AI_bonusVal(eBonus, iChange);
...
int CvPlayerAI::AI_stopTradingTradeVal(TeamTypes eTradeTeam, PlayerTypes ePlayer) const {
	...
	switch (kTheirTeam.AI_getAttitude(eTradeTeam)) {
	...
	if (kTheirTeam.isOpenBorders(eTradeTeam)) {
		iValue *= 2;
	}

	if (kTheirTeam.isDefensivePact(eTradeTeam)) {
		iValue *= 3;
	}

	if (kTheirTeam.isHasEmbassy(eTradeTeam)) {
		iValue *= 2;
	}
	...
	for (CvDeal* pLoopDeal = GC.getGameINLINE().firstDeal(&iLoop); pLoopDeal != NULL; pLoopDeal = GC.getGameINLINE().nextDeal(&iLoop)) {
		if (pLoopDeal->isCancelable(getID()) && !(pLoopDeal->isPeaceDeal())) {
			if (GET_PLAYER(pLoopDeal->getFirstPlayer()).getTeam() == kTheirPlayer.getTeam()) {
			...
			if (GET_PLAYER(pLoopDeal->getSecondPlayer()).getTeam() == kTheirPlayer.getTeam()) {
	...
	if (kTheirTeam.isVassal(getTeam())) {
		iValue /= 2;
	}
...
DenialTypes CvPlayerAI::AI_stopTradingTrade(TeamTypes eTradeTeam, PlayerTypes ePlayer) const {
	...
	AttitudeTypes eAttitude = GET_TEAM(getTeam()).AI_getAttitude(GET_PLAYER(ePlayer).getTeam());
...
int CvPlayerAI::AI_unitValue(UnitTypes eUnit, UnitAITypes eUnitAI, CvArea* pArea) const {
		...
		case UNITAI_MISSIONARY:
			if (pArea != NULL) {
				for (int iI = 0; iI < GC.getNumReligionInfos(); iI++) {
					if (kUnit.getReligionSpreads((ReligionTypes)iI) > 0) {
	...
	case UNITAI_RESERVE:
		iValue += iCombatValue;
		for (int iI = 0; iI < GC.getNumUnitCombatInfos(); iI++) {
			iValue += ((iCombatValue * kUnit.getUnitCombatModifier(iI) * AI_getUnitCombatWeight((UnitCombatTypes)iI)) / 12000);
		}
		if (kUnit.getMoves() > 1) {
	...
	case UNITAI_PARADROP:
		...
		for (int iI = 0; iI < GC.getNumUnitClassInfos(); iI++) {
			iValue += ((iCombatValue * kUnit.getUnitClassAttackModifier(iI) * AI_getUnitClassWeight((UnitClassTypes)iI)) / 10000);
			iValue += ((iCombatValue * (kUnit.getDefenderUnitClass(iI) ? 50 : 0)) / 100);
		}
		for (int iI = 0; iI < GC.getNumUnitCombatInfos(); iI++) {
			iValue += ((iCombatValue * kUnit.getUnitCombatModifier(iI) * AI_getUnitCombatWeight((UnitCombatTypes)iI)) / 10000);
			iValue += ((iCombatValue * (kUnit.getDefenderUnitCombat(iI) ? 50 : 0)) / 100);
		}
		if (kUnit.getInterceptionProbability() > 0) {
			int iTempValue = kUnit.getInterceptionProbability();
	...
	case UNITAI_MISSIONARY:
		...
		for (int iI = 0; iI < GC.getNumReligionInfos(); iI++) {
			if (kUnit.getReligionSpreads((ReligionTypes)iI) && hasHolyCity((ReligionTypes)iI)) {
				iValue += 80;
				break;
			}
		}
		if (AI_isDoVictoryStrategy(AI_VICTORY_CULTURE2)) {
			int iTempValue = 0;
			for (int iI = 0; iI < GC.getNumReligionInfos(); iI++) {
				if (kUnit.getReligionSpreads((ReligionTypes)iI)) {
	...
	case UNITAI_ICBM:
		if (kUnit.getNukeRange() != -1) {
			int iTempValue = 100 + (kUnit.getNukeRange() * 40);
...
int CvPlayerAI::AI_executiveValue(CvArea* pArea, CorporationTypes eCorporation, PlayerTypes* peBestPlayer, bool bSpreadOnly) const {
	...
	for (int iPlayer = 0; iPlayer < MAX_PLAYERS; iPlayer++) {
		...
		if (kLoopPlayer.isAlive() && iNumCities > 0) {
			if (GET_TEAM(getTeam()).isFriendlyTerritory(kLoopPlayer.getTeam()) || GET_TEAM(kLoopPlayer.getTeam()).isOpenBorders(getTeam()) || GET_TEAM(kLoopPlayer.getTeam()).isLimitedBorders(getTeam())) {
...
ReligionTypes CvPlayerAI::AI_bestReligion() const {
	...
	// K-Mod. Don't instantly convert to the first religion available, unless it is your own religion.
		...
		if (GET_PLAYER((PlayerTypes)iI).isAlive()) {
			if (GET_PLAYER((PlayerTypes)iI).getTeam() != getTeam()) {
				if (GET_TEAM(getTeam()).isHasMet(GET_PLAYER((PlayerTypes)iI).getTeam())) {
					...
					int iBonusImports = getNumTradeBonusImports((PlayerTypes)iI);
	...
	if (!GC.getGameINLINE().isOption(GAMEOPTION_NO_ESPIONAGE) && kTeam.getHasMetCivCount(true) > 0 && (isCommerceFlexible(COMMERCE_ESPIONAGE) || getCommerceRate(COMMERCE_ESPIONAGE) > 0)) {
		...
		for (int iTeam = 0; iTeam < MAX_CIV_TEAMS; ++iTeam) {
			...
			if (kLoopTeam.isAlive() && iTeam != getTeam() && kTeam.isHasMet((TeamTypes)iTeam) &&
				...
				if (iTheirEspPoints > iDesiredMissionPoints && kTeam.AI_getAttitude((TeamTypes)iTeam) <= ATTITUDE_CAUTIOUS) {
					iRateDivisor += 1;
		...
		for (int iTeam = 0; iTeam < MAX_CIV_TEAMS; ++iTeam) {
			if (aiTarget[iTeam] > 0) {
				...
				if (iTeam != eMinModTeam && (!AI_isMaliciousEspionageTarget(GET_TEAM((TeamTypes)iTeam).getLeaderID()) || !kTeam.AI_hasCitiesInPrimaryArea((TeamTypes)iTeam)))
					aiWeight[iTeam] = 0;
			}
...
void CvPlayerAI::AI_doDiplo() {
	...
	for (int iPass = 0; iPass < 2; iPass++) {
		for (PlayerTypes eOtherPlayer = (PlayerTypes)0; eOtherPlayer < MAX_CIV_PLAYERS; eOtherPlayer = (PlayerTypes)(eOtherPlayer + 1)) {
			const CvTeamAI& kOurTeam = GET_TEAM(getTeam());
			const CvPlayerAI& kOtherPlayer = GET_PLAYER(eOtherPlayer);
			TeamTypes eOtherTeam = kOtherPlayer.getTeam();
			const CvTeamAI& kOtherTeam = GET_TEAM(eOtherTeam);
			if (kOtherPlayer.isAlive()) {
				if (kOtherPlayer.isHuman() == (iPass == 1)) {
					if (eOtherPlayer != getID()) {
						...
						for (CvDeal* pLoopDeal = GC.getGameINLINE().firstDeal(&iLoop); pLoopDeal != NULL; pLoopDeal = GC.getGameINLINE().nextDeal(&iLoop)) {
							if (pLoopDeal->isCancelable(getID())) {
								...
								if ((pLoopDeal->getFirstPlayer() == getID()) && (pLoopDeal->getSecondPlayer() == eOtherPlayer)) {
									if (!AI_considerOffer(eOtherPlayer, pLoopDeal->getSecondTrades(), pLoopDeal->getFirstTrades(), -1)) {
										bCancelDeal = true;
									}
								} else if ((pLoopDeal->getFirstPlayer() == eOtherPlayer) && (pLoopDeal->getSecondPlayer() == getID())) {
									if (!AI_considerOffer(eOtherPlayer, pLoopDeal->getFirstTrades(), pLoopDeal->getSecondTrades(), -1)) {
										bCancelDeal = true;
									}
								}

								if (bCancelDeal) {
									if (kOtherPlayer.isHuman() && canContactAndTalk(eOtherPlayer)) {
										ourList.clear();
										theirList.clear();
										...
										for (PlayerTypes eLoopPlayer = (PlayerTypes)0; eLoopPlayer < MAX_PLAYERS; eLoopPlayer = (PlayerTypes)(eLoopPlayer + 1)) {
											...
											if (kLoopPLayer.isAlive()) {
												if (getTeam() == kLoopPLayer.getTeam()) {
													kLoopPLayer.AI_changeMemoryCount(eLoopPlayer, MEMORY_RECALLED_AMBASSADOR, -AI_getMemoryCount(eLoopPlayer, MEMORY_RECALLED_AMBASSADOR));
												}
											}
										}
									}
						...
						if (canContactAndTalk(eOtherPlayer)) {
							TradeData item;
							if (eOtherTeam == getTeam() || kOtherTeam.isVassal(getTeam())) {
								...
								for (BonusTypes eBonus = (BonusTypes)0; eBonus < GC.getNumBonusInfos(); eBonus = (BonusTypes)(eBonus + 1)) {
									if (getNumTradeableBonuses(eBonus) > 1) {
										bool bHasBonus = kOtherPlayer.getNumAvailableBonuses(eBonus) > 0;
										if (kOtherPlayer.AI_bonusTradeVal(eBonus, getID(), 1) > 0
											&& 2 * kOtherPlayer.AI_bonusVal(eBonus, 1) > (bHasBonus ? 3 : 2) * AI_bonusVal(eBonus, -1)) {
											setTradeItem(&item, TRADE_RESOURCES, eBonus);

											if (canTradeItem(eOtherPlayer, item, true)) {
												int iValue = (1 + GC.getGameINLINE().getSorenRandNum(10000, "AI Bonus Trading #1"));
							...
							if (eOtherTeam != getTeam() && kOtherTeam.isVassal(getTeam())) {
								...
								if (kOtherPlayer.getTechScore() * 10 < getTechScore() * (kOtherPlayer.isHuman() ? 7 : 9)) // K-Mod
								{
									for (TechTypes eTech = (TechTypes)0; eTech < GC.getNumTechInfos(); eTech = (TechTypes)(eTech + 1)) {
										if (kOurTeam.AI_techTrade(eTech, eOtherTeam) == NO_DENIAL) {
											setTradeItem(&item, TRADE_TECHNOLOGIES, eTech);

											if (canTradeItem(eOtherPlayer, item, true)) {
												int iValue = (1 + GC.getGameINLINE().getSorenRandNum(10000, "AI Vassal Tech gift"));
							...
							if (eOtherTeam != getTeam() && !((kOurTeam).isHuman()) && (kOtherPlayer.isHuman() || !(kOtherTeam.isHuman()))) {
								...
								if (kOtherTeam.isVassal(getTeam())) {
									...
									for (BonusTypes eBonus = (BonusTypes)0; eBonus < GC.getNumBonusInfos(); eBonus = (BonusTypes)(eBonus + 1)) {
										if (kOtherPlayer.getNumTradeableBonuses(eBonus) > 0 && getNumAvailableBonuses(eBonus) == 0) {
											int iValue = AI_bonusTradeVal(eBonus, eOtherPlayer, 1);
									...
									if (eBestGiveBonus != NO_BONUS) {
										...
										if (kOtherPlayer.isHuman()) {
											if (!(abContacted[eOtherTeam])) {
												...
												if (pInfo) {
													gDLL->getInterfaceIFace()->addPopup(pInfo, eOtherPlayer);
													abContacted[eOtherTeam] = true;
												}
											}
										} else {
								...
								if (!kOurTeam.isAtWar(eOtherTeam)) {
									if (AI_getAttitude(eOtherPlayer) >= ATTITUDE_CAUTIOUS) {
										int iLoop;
										for (CvCity* pLoopCity = firstCity(&iLoop); pLoopCity != NULL; pLoopCity = nextCity(&iLoop)) {
											if (pLoopCity->getPreviousOwner() != eOtherPlayer) {
												if (((pLoopCity->getGameTurnAcquired() + 4) % 20) == (GC.getGameINLINE().getGameTurn() % 20)) {
													...
													for (int iJ = 0; iJ < NUM_CITY_PLOTS; iJ++) {
														...
														if (pLoopPlot != NULL) {
															if (pLoopPlot->getOwnerINLINE() == eOtherPlayer) {
																iCount++;
															}
													...
													if (iCount >= (iPossibleCount / 2)) {
														...
														if (canTradeItem(eOtherPlayer, item, true)) {
															...
															if (kOtherPlayer.isHuman()) {
																...
																GC.getGameINLINE().implementDeal(getID(), eOtherPlayer, &ourList, NULL);
															}
														}
													}
												}
									...
									if (kOurTeam.getLeaderID() == getID()) {
										if (AI_getContactTimer(eOtherPlayer, CONTACT_PERMANENT_ALLIANCE) == 0) {
											bool bOffered = false;
											if (GC.getGameINLINE().getSorenRandNum(GC.getLeaderHeadInfo(getPersonalityType()).getContactRand(CONTACT_PERMANENT_ALLIANCE), "AI Diplo Alliance") == 0) {
												...
												if (canTradeItem(eOtherPlayer, item, true) && kOtherPlayer.canTradeItem(getID(), item, true)) {
													...
													if (kOtherPlayer.isHuman()) {
														if (!(abContacted[eOtherTeam])) {
															AI_changeContactTimer(eOtherPlayer, CONTACT_PERMANENT_ALLIANCE, GC.getLeaderHeadInfo(getPersonalityType()).getContactDelay(CONTACT_PERMANENT_ALLIANCE));
															CvDiploParameters* pDiplo = new CvDiploParameters(getID());
															FAssertMsg(pDiplo != NULL, "pDiplo must be valid");
											...
											if (!bOffered) {
												...
												if (canTradeItem(eOtherPlayer, item, true)) {
													...
													if (kOtherPlayer.isHuman()) {
														...
														bool bAccepted = true;
														TeamTypes eMasterTeam = eOtherTeam;
														for (TeamTypes eLoopTeam = (TeamTypes)0; eLoopTeam < MAX_CIV_TEAMS; eLoopTeam = (TeamTypes)(eLoopTeam + 1)) {
															if (GET_TEAM(eLoopTeam).isAlive()) {
																if (eLoopTeam != getTeam() && eLoopTeam != eMasterTeam && atWar(getTeam(), eLoopTeam) && !atWar(eMasterTeam, eLoopTeam)) {
																	if (GET_TEAM(eMasterTeam).AI_declareWarTrade(eLoopTeam, getTeam(), false) != NO_DENIAL) {
																		bAccepted = false;
																		break;
																	}
																}
									...
									if (kOtherPlayer.isHuman() && (kOurTeam.getLeaderID() == getID()) && !kOurTeam.isVassal(eOtherTeam)) {
										if (getStateReligion() != NO_RELIGION) {
											if (kOtherPlayer.canConvert(getStateReligion())) {
												if (AI_getContactTimer(eOtherPlayer, CONTACT_RELIGION_PRESSURE) == 0) {
													if (GC.getGameINLINE().getSorenRandNum(GC.getLeaderHeadInfo(getPersonalityType()).getContactRand(CONTACT_RELIGION_PRESSURE), "AI Diplo Religion Pressure") == 0) {
														if (!(abContacted[eOtherTeam])) {
															AI_changeContactTimer(eOtherPlayer, CONTACT_RELIGION_PRESSURE, GC.getLeaderHeadInfo(getPersonalityType()).getContactDelay(CONTACT_RELIGION_PRESSURE));
															CvDiploParameters* pDiplo = new CvDiploParameters(getID());
															FAssertMsg(pDiplo != NULL, "pDiplo must be valid");
									...
									if (kOtherPlayer.isHuman() && (kOurTeam.getLeaderID() == getID()) && !kOurTeam.isVassal(eOtherTeam)) {
										...
										if (eFavoriteCivic != NO_CIVIC) {
											if (isCivic(eFavoriteCivic)) {
												if (kOtherPlayer.canDoCivics(eFavoriteCivic) && !(kOtherPlayer.isCivic(eFavoriteCivic))) {
													if (kOtherPlayer.canRevolution(NULL)) {
														if (AI_getContactTimer(eOtherPlayer, CONTACT_CIVIC_PRESSURE) == 0) {
															if (GC.getGameINLINE().getSorenRandNum(GC.getLeaderHeadInfo(getPersonalityType()).getContactRand(CONTACT_CIVIC_PRESSURE), "AI Diplo Civic Pressure") == 0) {
																if (!(abContacted[eOtherTeam])) {
																	AI_changeContactTimer(eOtherPlayer, CONTACT_CIVIC_PRESSURE, GC.getLeaderHeadInfo(getPersonalityType()).getContactDelay(CONTACT_CIVIC_PRESSURE));
																	CvDiploParameters* pDiplo = new CvDiploParameters(getID());
																	FAssertMsg(pDiplo != NULL, "pDiplo must be valid");
									...
									if (kOtherPlayer.isHuman() && (kOurTeam.getLeaderID() == getID())) {
										if ((AI_getMemoryCount(eOtherPlayer, MEMORY_DECLARED_WAR) == 0) && (AI_getMemoryCount(eOtherPlayer, MEMORY_HIRED_WAR_ALLY) == 0)) {
											if (AI_getContactTimer(eOtherPlayer, CONTACT_JOIN_WAR) == 0) {
												if (GC.getGameINLINE().getSorenRandNum(GC.getLeaderHeadInfo(getPersonalityType()).getContactRand(CONTACT_JOIN_WAR), "AI Diplo Join War") == 0) {
													...
													for (TeamTypes eLoopTeam = (TeamTypes)0; eLoopTeam < MAX_CIV_TEAMS; eLoopTeam = (TeamTypes)(eLoopTeam + 1)) {
														if (GET_TEAM(eLoopTeam).isAlive()) {
															if (atWar(eLoopTeam, getTeam()) && !atWar(eLoopTeam, eOtherTeam)) {
																if (kOtherTeam.isHasMet(eLoopTeam)) {
																	if (kOtherTeam.canDeclareWar(eLoopTeam)) {
													...
													if (eBestTeam != NO_TEAM) {
														if (!(abContacted[eOtherTeam])) {
															AI_changeContactTimer(eOtherPlayer, CONTACT_JOIN_WAR, GC.getLeaderHeadInfo(getPersonalityType()).getContactDelay(CONTACT_JOIN_WAR));
															CvDiploParameters* pDiplo = new CvDiploParameters(getID());
															FAssertMsg(pDiplo != NULL, "pDiplo must be valid");
									...
									if (kOtherPlayer.isHuman() && (kOurTeam.getLeaderID() == getID()) && !kOurTeam.isVassal(eOtherTeam)) {
										if (AI_getContactTimer(eOtherPlayer, CONTACT_STOP_TRADING) == 0) {
											if (GC.getGameINLINE().getSorenRandNum(GC.getLeaderHeadInfo(getPersonalityType()).getContactRand(CONTACT_STOP_TRADING), "AI Diplo Stop Trading") == 0) {
												...
												if ((eBestTeam != NO_TEAM) && kOtherTeam.isHasMet(eBestTeam) && !GET_TEAM(eBestTeam).isVassal(eOtherTeam)) {
													if (kOtherPlayer.canStopTradingWithTeam(eBestTeam)) {
														FAssert(!atWar(eOtherTeam, eBestTeam));
														FAssert(eOtherTeam != eBestTeam);

														if (!(abContacted[eOtherTeam])) {
															AI_changeContactTimer(eOtherPlayer, CONTACT_STOP_TRADING, GC.getLeaderHeadInfo(getPersonalityType()).getContactDelay(CONTACT_STOP_TRADING));
															CvDiploParameters* pDiplo = new CvDiploParameters(getID());
															FAssertMsg(pDiplo != NULL, "pDiplo must be valid");
									...
									if (kOtherPlayer.isHuman() && (kOurTeam.getLeaderID() == getID())) {
										if (kOtherTeam.getAssets() < (kOurTeam.getAssets() / 2)) {
											if (AI_getAttitude(eOtherPlayer) > GC.getLeaderHeadInfo(kOtherPlayer.getPersonalityType()).getNoGiveHelpAttitudeThreshold()) {
												if (AI_getContactTimer(eOtherPlayer, CONTACT_GIVE_HELP) == 0) {
													if (GC.getGameINLINE().getSorenRandNum(GC.getLeaderHeadInfo(getPersonalityType()).getContactRand(CONTACT_GIVE_HELP), "AI Diplo Give Help") == 0) {
														...
														for (TechTypes eTech = (TechTypes)0; eTech < GC.getNumTechInfos(); eTech = (TechTypes)(eTech + 1)) {
															...
															if (canTradeItem(eOtherPlayer, item, true)) {
																int iValue = (1 + GC.getGameINLINE().getSorenRandNum(10000, "AI Giving Help"));
														...
														if (eBestGiveTech != NO_TECH) {
															...
															if (!(abContacted[eOtherTeam])) {
																AI_changeContactTimer(eOtherPlayer, CONTACT_GIVE_HELP, GC.getLeaderHeadInfo(getPersonalityType()).getContactDelay(CONTACT_GIVE_HELP));
																CvDiploParameters* pDiplo = new CvDiploParameters(getID());
																FAssertMsg(pDiplo != NULL, "pDiplo must be valid");
																pDiplo->setDiploComment((DiploCommentTypes)GC.getInfoTypeForString("AI_DIPLOCOMMENT_GIVE_HELP"));
																pDiplo->setAIContact(true);
									...
									if (kOtherPlayer.isHuman() && (kOurTeam.getLeaderID() == getID())) {
										if (kOtherTeam.getAssets() > (kOurTeam.getAssets() / 2)) {
											if (AI_getContactTimer(eOtherPlayer, CONTACT_ASK_FOR_HELP) == 0) {
												...
												if (GC.getGameINLINE().getSorenRandNum(iRand, "AI Diplo Ask For Help") == 0) {
													...
													if (eBestReceiveTech != NO_TECH) {
														...
														if (!(abContacted[eOtherTeam])) {
															AI_changeContactTimer(eOtherPlayer, CONTACT_ASK_FOR_HELP, GC.getLeaderHeadInfo(getPersonalityType()).getContactDelay(CONTACT_ASK_FOR_HELP));
															CvDiploParameters* pDiplo = new CvDiploParameters(getID());
															FAssertMsg(pDiplo != NULL, "pDiplo must be valid");
									...
									if (kOtherPlayer.isHuman() && (kOurTeam.getLeaderID() == getID())) {
										...
										if (kOurTeam.canDeclareWar(eOtherTeam) && !kOurTeam.AI_isChosenWar(eOtherTeam)) {
											if (kOtherTeam.getDefensivePower() < kOurTeam.getPower(true)) {
												if (AI_getAttitude(eOtherPlayer) <= GC.getLeaderHeadInfo(kOtherPlayer.getPersonalityType()).getDemandTributeAttitudeThreshold()) {
													if (AI_getContactTimer(eOtherPlayer, CONTACT_DEMAND_TRIBUTE) == 0) {
														if (GC.getGameINLINE().getSorenRandNum(GC.getLeaderHeadInfo(getPersonalityType()).getContactRand(CONTACT_DEMAND_TRIBUTE), "AI Diplo Demand Tribute") == 0) {
															int iReceiveGold = std::min(std::max(0, (kOtherPlayer.getGold() - 50)), kOtherPlayer.AI_goldTarget());
															...
															if (iReceiveGold > 50) {
																...
																if (!(abContacted[eOtherTeam])) {
																	AI_changeContactTimer(eOtherPlayer, CONTACT_DEMAND_TRIBUTE, GC.getLeaderHeadInfo(getPersonalityType()).getContactDelay(CONTACT_DEMAND_TRIBUTE));
																	CvDiploParameters* pDiplo = new CvDiploParameters(getID());
																	FAssertMsg(pDiplo != NULL, "pDiplo must be valid");
														...
														if (GC.getGameINLINE().getSorenRandNum(GC.getLeaderHeadInfo(getPersonalityType()).getContactRand(CONTACT_DEMAND_TRIBUTE), "AI Diplo Demand Tribute") == 0) {
															if (kOurTeam.AI_mapTradeVal(eOtherTeam) > 100) {
																theirList.clear();
															...
															for (TechTypes eTech = (TechTypes)0; eTech < GC.getNumTechInfos(); eTech = (TechTypes)(eTech + 1)) {
																...
																if (kOtherPlayer.canTradeItem(getID(), item, true)) {
																	if (GC.getGameINLINE().countKnownTechNumTeams(eTech) > 1) {
																		int iValue = (1 + GC.getGameINLINE().getSorenRandNum(10000, "AI Demanding Tribute (Tech)"));
														...
														if (GC.getGameINLINE().getSorenRandNum(GC.getLeaderHeadInfo(getPersonalityType()).getContactRand(CONTACT_DEMAND_TRIBUTE), "AI Diplo Demand Tribute") == 0) {
															...
															for (BonusTypes eBonus = (BonusTypes)0; eBonus < GC.getNumBonusInfos(); eBonus = (BonusTypes)(eBonus + 1)) {
																if (kOtherPlayer.getNumTradeableBonuses(eBonus) > 1) {
																	if (AI_bonusTradeVal(eBonus, eOtherPlayer, 1) > 0) {
																		setTradeItem(&item, TRADE_RESOURCES, eBonus);

																		if (kOtherPlayer.canTradeItem(getID(), item, true)) {
																			int iValue = (1 + GC.getGameINLINE().getSorenRandNum(10000, "AI Demanding Tribute (Bonus)"));
									...
									if (kOurTeam.getLeaderID() == getID()) {
										if (AI_getContactTimer(eOtherPlayer, CONTACT_OPEN_BORDERS) == 0) {
											if (GC.getGameINLINE().getSorenRandNum(GC.getLeaderHeadInfo(getPersonalityType()).getContactRand(CONTACT_OPEN_BORDERS), "AI Diplo Open Borders") == 0) {
												...
												if (canTradeItem(eOtherPlayer, item, true) && kOtherPlayer.canTradeItem(getID(), item, true)) {
													...
													if (kOtherPlayer.isHuman()) {
														if (!(abContacted[eOtherTeam])) {
															AI_changeContactTimer(eOtherPlayer, CONTACT_OPEN_BORDERS, GC.getLeaderHeadInfo(getPersonalityType()).getContactDelay(CONTACT_OPEN_BORDERS));
															CvDiploParameters* pDiplo = new CvDiploParameters(getID());
															FAssertMsg(pDiplo != NULL, "pDiplo must be valid");
															pDiplo->setDiploComment((DiploCommentTypes)GC.getInfoTypeForString("AI_DIPLOCOMMENT_OFFER_DEAL"));
															pDiplo->setAIContact(true);
									...
									if (kOurTeam.getLeaderID() == getID()) {
										if (AI_getContactTimer(eOtherPlayer, CONTACT_DEFENSIVE_PACT) == 0) {
											if (GC.getGameINLINE().getSorenRandNum(GC.getLeaderHeadInfo(getPersonalityType()).getContactRand(CONTACT_DEFENSIVE_PACT), "AI Diplo Defensive Pact") == 0) {
												...
												if (canTradeItem(eOtherPlayer, item, true) && kOtherPlayer.canTradeItem(getID(), item, true)) {
													...
													if (kOtherPlayer.isHuman()) {
														if (!(abContacted[eOtherTeam])) {
															AI_changeContactTimer(eOtherPlayer, CONTACT_DEFENSIVE_PACT, GC.getLeaderHeadInfo(getPersonalityType()).getContactDelay(CONTACT_DEFENSIVE_PACT));
															CvDiploParameters* pDiplo = new CvDiploParameters(getID());
															FAssertMsg(pDiplo != NULL, "pDiplo must be valid");
															pDiplo->setDiploComment((DiploCommentTypes)GC.getInfoTypeForString("AI_DIPLOCOMMENT_OFFER_DEAL"));
															pDiplo->setAIContact(true);
									...
									if (!(kOtherPlayer.isHuman()) || (kOurTeam.getLeaderID() == getID())) {
										if (AI_getContactTimer(eOtherPlayer, CONTACT_TRADE_TECH) == 0) {
											...
											if (GC.getGameINLINE().getSorenRandNum(iRand, "AI Diplo Trade Tech") == 0) {
												...
												for (TechTypes eTech = (TechTypes)0; eTech < GC.getNumTechInfos(); eTech = (TechTypes)(eTech + 1)) {
													...
													if (kOtherPlayer.canTradeItem(getID(), item, true)) {
														int iValue = (1 + GC.getGameINLINE().getSorenRandNum(10000, "AI Tech Trading #1"));
												...
												if (eBestReceiveTech != NO_TECH) {
													int iOurValue = kOurTeam.AI_techTradeVal(eBestReceiveTech, eOtherTeam);
													int iTheirValue = 0;
													...
													for (TechTypes eTech = (TechTypes)0; eTech < GC.getNumTechInfos(); eTech = (TechTypes)(eTech + 1)) {
														...
														if (canTradeItem(eOtherPlayer, item, true)) {
															int iValue = kOtherTeam.AI_techTradeVal(eTech, getTeam());
													...
													bool bDeal = 100 * iOurValue >= AI_tradeAcceptabilityThreshold(eOtherPlayer) * iTheirValue && 100 * iTheirValue >= kOtherPlayer.AI_tradeAcceptabilityThreshold(getID()) * iOurValue;
													if (!bDeal) {
														...
														buildTradeTable(eOtherPlayer, ourInventory); // all tradeable items
														markTradeOffers(ourInventory, ourList); // K-Mod funciton - set m_bOffering on each item offered
														updateTradeList(eOtherPlayer, ourInventory, ourList, theirList); // hide what should be excluded
														kOtherPlayer.buildTradeTable(getID(), theirInventory); // same for the other player
														kOtherPlayer.markTradeOffers(theirInventory, theirList);
														kOtherPlayer.updateTradeList(getID(), theirInventory, theirList, ourList);
														...
														if (kOtherPlayer.AI_counterPropose(getID(), &ourList, &theirList, &ourInventory, &theirInventory, &ourCounter, &theirCounter)) {
															...
															if (gTeamLogLevel >= 2) logBBAI("    %S makes a deal with %S using AI_counterPropose. (%d : %d)", getName(0), kOtherPlayer.getName(0), ourList.getLength(), theirList.getLength());
														}
													}
													if (bDeal) {
														if (kOtherPlayer.isHuman()) {
															if (!(abContacted[eOtherTeam])) {
																AI_changeContactTimer(eOtherPlayer, CONTACT_TRADE_TECH, GC.getLeaderHeadInfo(getPersonalityType()).getContactDelay(CONTACT_TRADE_TECH));
																CvDiploParameters* pDiplo = new CvDiploParameters(getID());
																FAssertMsg(pDiplo != NULL, "pDiplo must be valid");
																pDiplo->setDiploComment((DiploCommentTypes)GC.getInfoTypeForString("AI_DIPLOCOMMENT_OFFER_DEAL"));
																pDiplo->setAIContact(true);
									...
									if (kOurTeam.getLeaderID() == getID()) {
										if (!kOurTeam.isHasEmbassy(eOtherTeam)) {
											if (AI_getContactTimer(eOtherPlayer, CONTACT_EMBASSY) == 0) {
												if (GC.getGameINLINE().getSorenRandNum(GC.getLeaderHeadInfo(getPersonalityType()).getContactRand(CONTACT_EMBASSY), "AI Diplo Open Borders") == 0) {
													...
													if (canTradeItem(eOtherPlayer, item, true) && kOtherPlayer.canTradeItem(getID(), item, true)) {
														...
														if (kOtherPlayer.isHuman()) {
															if (!(abContacted[eOtherTeam])) {
																AI_changeContactTimer(eOtherPlayer, CONTACT_EMBASSY, GC.getLeaderHeadInfo(getPersonalityType()).getContactDelay(CONTACT_EMBASSY));
																CvDiploParameters* pDiplo = new CvDiploParameters(getID());
																FAssertMsg(pDiplo != NULL, "pDiplo must be valid");
																pDiplo->setDiploComment((DiploCommentTypes)GC.getInfoTypeForString("AI_DIPLOCOMMENT_OFFER_DEAL"));
																pDiplo->setAIContact(true);
									...
									if (AI_getContactTimer(eOtherPlayer, CONTACT_TRADE_BONUS) == 0) {
										if (GC.getGameINLINE().getSorenRandNum(GC.getLeaderHeadInfo(getPersonalityType()).getContactRand(CONTACT_TRADE_BONUS), "AI Diplo Trade Bonus") == 0) {
											...
											for (BonusTypes eBonus = (BonusTypes)0; eBonus < GC.getNumBonusInfos(); eBonus = (BonusTypes)(eBonus + 1)) {
												if (kOtherPlayer.getNumTradeableBonuses(eBonus) > 1) {
													if (kOtherPlayer.AI_corporationBonusVal(eBonus) == 0) {
														if (AI_bonusTradeVal(eBonus, eOtherPlayer, 1) > 0) {
															setTradeItem(&item, TRADE_RESOURCES, eBonus);
											...
											if (eBestReceiveBonus != NO_BONUS) {
												...
												for (BonusTypes eBonus = (BonusTypes)0; eBonus < GC.getNumBonusInfos(); eBonus = (BonusTypes)(eBonus + 1)) {
													if (eBonus != eBestReceiveBonus) {
														if (getNumTradeableBonuses(eBonus) > 1) {
															if (kOtherPlayer.AI_bonusTradeVal(eBonus, getID(), 1) > 0) {
																setTradeItem(&item, TRADE_RESOURCES, eBonus);

																if (canTradeItem(eOtherPlayer, item, true)) {
																	int iValue = (1 + GC.getGameINLINE().getSorenRandNum(10000, "AI Bonus Trading #2"));
												...
												if (eBestGiveBonus != NO_BONUS) {
													if (!(kOtherPlayer.isHuman()) || (AI_bonusTradeVal(eBestReceiveBonus, eOtherPlayer, -1) >= kOtherPlayer.AI_bonusTradeVal(eBestGiveBonus, getID(), 1))) {
														ourList.clear();
														theirList.clear();
														...
														if (kOtherPlayer.isHuman()) {
															if (!(abContacted[eOtherTeam])) {
																AI_changeContactTimer(eOtherPlayer, CONTACT_TRADE_BONUS, GC.getLeaderHeadInfo(getPersonalityType()).getContactDelay(CONTACT_TRADE_BONUS));
																CvDiploParameters* pDiplo = new CvDiploParameters(getID());
																FAssertMsg(pDiplo != NULL, "pDiplo must be valid");
																pDiplo->setDiploComment((DiploCommentTypes)GC.getInfoTypeForString("AI_DIPLOCOMMENT_OFFER_DEAL"));
																pDiplo->setAIContact(true);
									...
									if (AI_getContactTimer(eOtherPlayer, CONTACT_TRADE_MAP) == 0) {
										if (GC.getGameINLINE().getSorenRandNum(GC.getLeaderHeadInfo(getPersonalityType()).getContactRand(CONTACT_TRADE_MAP), "AI Diplo Trade Map") == 0) {
											...
											if (kOtherPlayer.canTradeItem(getID(), item, true) && canTradeItem(eOtherPlayer, item, true)) {
												if (!(kOtherPlayer.isHuman()) || (kOurTeam.AI_mapTradeVal(eOtherTeam) >= kOtherTeam.AI_mapTradeVal(getTeam()))) {
													ourList.clear();
													theirList.clear();
													...
													if (kOtherPlayer.isHuman()) {
														if (!(abContacted[eOtherTeam])) {
															AI_changeContactTimer(eOtherPlayer, CONTACT_TRADE_MAP, GC.getLeaderHeadInfo(getPersonalityType()).getContactDelay(CONTACT_TRADE_MAP));
															CvDiploParameters* pDiplo = new CvDiploParameters(getID());
															FAssertMsg(pDiplo != NULL, "pDiplo must be valid");
															pDiplo->setDiploComment((DiploCommentTypes)GC.getInfoTypeForString("AI_DIPLOCOMMENT_OFFER_DEAL"));
															pDiplo->setAIContact(true);
									...
									if (!(kOtherPlayer.isHuman()) || !(abContacted[eOtherTeam])) {
										int iDeclareWarTradeRand = GC.getLeaderHeadInfo(getPersonalityType()).getDeclareWarTradeRand();
										int iMinAtWarCounter = MAX_INT;
										for (TeamTypes eLoopTeam = (TeamTypes)0; eLoopTeam < MAX_CIV_TEAMS; eLoopTeam = (TeamTypes)(eLoopTeam + 1)) {
											if (GET_TEAM(eLoopTeam).isAlive()) {
												if (atWar(eLoopTeam, getTeam())) {
													int iAtWarCounter = kOurTeam.AI_getAtWarCounter(eLoopTeam);
													if (kOurTeam.AI_getWarPlan(eLoopTeam) == WARPLAN_DOGPILE) {
														iAtWarCounter *= 2;
														iAtWarCounter += 5;
													}
										...
										if (GC.getGameINLINE().getSorenRandNum(iDeclareWarTradeRand, "AI Diplo Declare War Trade") == 0) {
											...
											for (TeamTypes eLoopTeam = (TeamTypes)0; eLoopTeam < MAX_CIV_TEAMS; eLoopTeam = (TeamTypes)(eLoopTeam + 1)) {
												if (GET_TEAM(eLoopTeam).isAlive()) {
													if (atWar(eLoopTeam, getTeam()) && !atWar(eLoopTeam, eOtherTeam)) {
														if (GET_TEAM(eLoopTeam).getAtWarCount(true) < std::max(2, (GC.getGameINLINE().countCivTeamsAlive() / 2))) {
															setTradeItem(&item, TRADE_WAR, eLoopTeam);

															if (kOtherPlayer.canTradeItem(getID(), item, true)) {
																int iValue = (1 + GC.getGameINLINE().getSorenRandNum(1000, "AI Declare War Trading"));

																iValue *= (101 + GET_TEAM(eLoopTeam).AI_getAttitudeWeight(getTeam()));
																iValue /= 100;
											...
											if (eBestTeam != NO_TEAM) {
												...
												for (TechTypes eTech = (TechTypes)0; eTech < GC.getNumTechInfos(); eTech = (TechTypes)(eTech + 1)) {
													...
													if (canTradeItem(eOtherPlayer, item, true)) {
														int iValue = (1 + GC.getGameINLINE().getSorenRandNum(100, "AI Tech Trading #2"));
												...
												int iOurValue = kOurTeam.AI_declareWarTradeVal(eBestTeam, eOtherTeam);
												int iTheirValue = 0;
												if (eBestGiveTech != NO_TECH) {
													iTheirValue = kOtherTeam.AI_techTradeVal(eBestGiveTech, getTeam());
												} else {
													iTheirValue = 0;
												}
												...
												if ((iTheirValue < iOurValue) && (eBestGiveTech != NO_TECH)) {
													...
													if (eBestGiveTech2 != NO_TECH) {
														iTheirValue += kOtherTeam.AI_techTradeVal(eBestGiveTech2, getTeam());
													}
												}
												...
												if (iTheirValue > iOurValue) {
													int iGold = std::min(((iTheirValue - iOurValue) * 100) / iGoldValuePercent, kOtherPlayer.AI_maxGoldTrade(getID()));
													...
													int iGold = std::min(((iOurValue - iTheirValue) * 100) / iGoldValuePercent, AI_maxGoldTrade(eOtherPlayer));
...
// K-Mod, replaced this function with AI_disbandValue
...
int CvPlayerAI::AI_calculateDominationVictoryStage() const {
	...
	{
		int iValue = GC.getLeaderHeadInfo(getPersonalityType()).getDominationVictoryWeight();
...
int CvPlayerAI::AI_calculateDiplomacyVictoryStage() const {
	...
	{
		int iValue = GC.getLeaderHeadInfo(getPersonalityType()).getDiplomacyVictoryWeight();
...
#define log_strat2(s, x) \
	...
	for (UnitClassTypes eLoopUnitClass = (UnitClassTypes)0; eLoopUnitClass < GC.getNumUnitClassInfos(); eLoopUnitClass = (UnitClassTypes)(eLoopUnitClass + 1)) {
		UnitTypes eLoopUnit = ((UnitTypes)(GC.getCivilizationInfo(getCivilizationType()).getCivilizationUnits(eLoopUnitClass)));

		if (NO_UNIT != eLoopUnit) {
			if (getCapitalCity() != NULL) {
				if (getCapitalCity()->canTrain(eLoopUnit)) {
					CvUnitInfo& kLoopUnit = GC.getUnitInfo(eLoopUnit);
					bool bIsUU = (GC.getUnitClassInfo(eLoopUnitClass).getDefaultUnitIndex()) != (GC.getCivilizationInfo(getCivilizationType()).getCivilizationUnits(eLoopUnitClass));
					// K-Mod. Original code was missing the obvious: UNITAI_ATTACK. Was this a bug? (I'm skipping "pillage".)
					if (kLoopUnit.getUnitAIType(UNITAI_ATTACK) || kLoopUnit.getUnitAIType(UNITAI_ATTACK_CITY) || kLoopUnit.getUnitAIType(UNITAI_RESERVE) || kLoopUnit.getUnitAIType(UNITAI_COUNTER)) {
						iAttackUnitCount++;

						//UU love
						if (bIsUU) {
							if (kLoopUnit.getUnitAIType(UNITAI_ATTACK_CITY) || (kLoopUnit.getUnitAIType(UNITAI_ATTACK) && !kLoopUnit.getUnitAIType(UNITAI_CITY_DEFENSE))) {
								iAttackUnitCount++;
							}
						}
			...
			if ((iHolyCityCount > 0) && hasHolyCity(getStateReligion())) {
				...
				for (PlayerTypes eLoopPlayer = (PlayerTypes)0; eLoopPlayer < MAX_CIV_PLAYERS; eLoopPlayer = (PlayerTypes)(eLoopPlayer + 1)) {
					...
					if (eLoopPlayer != getID()) {
						if (kLoopPLayer.isAlive() && kTeam.isHasMet(kLoopPLayer.getTeam())) {
							if (kTeam.isOpenBorders(kLoopPLayer.getTeam()) || (kTeam.isLimitedBorders(kLoopPLayer.getTeam()))) {
								if (kLoopPLayer.getStateReligion() == getStateReligion()) {
									iMissionary += 10;
								} else if (!kLoopPLayer.isNoNonStateReligionSpread()) {
									iMissionary += (kLoopPLayer.countHolyCities() == 0) ? 12 : 4;
								}
							}
						}
					}
	...
	if (!GC.getGameINLINE().isOption(GAMEOPTION_NO_ESPIONAGE)) {
		...
		if (iLastStrategyHash & AI_STRATEGY_BIG_ESPIONAGE || AI_getNumAIUnits(UNITAI_SPY) > 0) {
			...
			if (iTempValue > 11) {
				...
				for (TeamTypes eLoopTeam = (TeamTypes)0; eLoopTeam < MAX_CIV_TEAMS; eLoopTeam = (TeamTypes)(eLoopTeam + 1)) {
					if (eLoopTeam != getTeam() && kTeam.isHasMet(eLoopTeam) && kTeam.AI_hasCitiesInPrimaryArea(eLoopTeam)) {
			...
			for (TeamTypes eLoopTeam = (TeamTypes)0; eLoopTeam < MAX_CIV_TEAMS; eLoopTeam = (TeamTypes)(eLoopTeam + 1)) {
				...
				if (eLoopTeam != getTeam()) {
					if (kLoopTeam.isAlive() && !kLoopTeam.isMinorCiv()) {
						iMaxWarCounter = std::max(iMaxWarCounter, kTeam.AI_getAtWarCounter(eLoopTeam));
					}
				}
			}
					...
					if (kTeam.AI_getWarPlan(kLoopPlayer.getTeam()) != NO_WARPLAN) {
						...
						if (4 * iTheirPower > 3 * iOurDefensivePower) {
							if (kLoopTeam.getAtWarCount(true) == 0 || kLoopTeam.AI_getEnemyPowerPercent(false) < 140) {
								...
								int iWarMemory = AI_getMemoryCount(eLoopPlayer, MEMORY_DECLARED_WAR);
								iWarMemory += (AI_getMemoryCount(eLoopPlayer, MEMORY_DECLARED_WAR_ON_FRIEND) + 1) / 2;
								...
								if (iWarMemory > 0) {
									...
									if (gPlayerLogLevel >= 2) {
										logBBAI("    Player %d (%S) wary of %S because of war memory %d", getID(), getCivilizationDescription(0), GET_PLAYER(eLoopPlayer).getCivilizationDescription(0), iWarMemory);
									}
								}
							}
						}
						...
						int iCloseness = AI_playerCloseness(eLoopPlayer, DEFAULT_PLAYER_CLOSENESS);
						if (iCloseness > 0 || bCitiesInPrime) // K-Mod
						{
							int iAttitudeWarProb = 100 - GC.getLeaderHeadInfo(getPersonalityType()).getNoWarAttitudeProb(AI_getAttitude(eLoopPlayer));
							...
							if (iTheirPower > 2 * iOurDefensivePower) {
								if (AI_getAttitude(eLoopPlayer) != ATTITUDE_FRIENDLY) {
									iTempParanoia += 25;
								}
							}
						}
		...
		if (!(AI_isDoVictoryStrategy(AI_VICTORY_CULTURE2))
			...
			for (UnitClassTypes eUnitClass = (UnitClassTypes)0; eUnitClass < GC.getNumUnitClassInfos(); eUnitClass = (UnitClassTypes)(eUnitClass + 1)) {
				...
				if ((eLoopUnit != NO_UNIT) && (GC.getUnitInfo(eLoopUnit).getCombat() > 0)) {
					...
					if (GC.getUnitClassInfo(eUnitClass).getDefaultUnitIndex() != GC.getCivilizationInfo(getCivilizationType()).getCivilizationUnits(eUnitClass)) {
						...
						if (getCapitalCity()->canTrain(eLoopUnit)) {
							...
							int iNormalStr = GC.getUnitInfo((UnitTypes)(GC.getUnitClassInfo(eUnitClass).getDefaultUnitIndex())).getCombat();
	...
	if (!(m_iStrategyHash & AI_STRATEGY_ALERT2) && !(m_iStrategyHash & AI_STRATEGY_TURTLE)) {//Crush
		int iWarCount = 0;

		int iCrushValue = AI_getStrategyRand(13) % (4 + AI_getFlavorValue(FLAVOR_MILITARY) / 2 + (GC.getGameINLINE().isOption(GAMEOPTION_AGGRESSIVE_AI) ? 2 : 0));
		...
		for (TeamTypes eOuterLoopTeam = (TeamTypes)0; eOuterLoopTeam < MAX_CIV_TEAMS; eOuterLoopTeam = (TeamTypes)(eOuterLoopTeam + 1)) {
			const CvTeam& kOuterLoopTeam = GET_TEAM(eOuterLoopTeam);
			if (kOuterLoopTeam.isAlive() && (eOuterLoopTeam != getTeam())) {
				if (kTeam.AI_getWarPlan(eOuterLoopTeam) != NO_WARPLAN) {
					if (!kOuterLoopTeam.isAVassal()) {
						if (kTeam.AI_teamCloseness(eOuterLoopTeam) > 0) {
							iWarCount++;
						}

						// K-Mod. (if we attack with our defenders, would they beat their defenders?)
						if (100 * iTypicalDefence >= 110 * kOuterLoopTeam.getTypicalUnitValue(UNITAI_CITY_DEFENSE)) {
							iCrushValue += 2;
						}
					}

					if (kTeam.AI_getWarPlan(eOuterLoopTeam) == WARPLAN_PREPARING_TOTAL) {
						iCrushValue += 6;
					} else if ((kTeam.AI_getWarPlan(eOuterLoopTeam) == WARPLAN_TOTAL) && (kTeam.AI_getWarPlanStateCounter(eOuterLoopTeam) < 20)) {
						iCrushValue += 6;
					}

					if ((kTeam.AI_getWarPlan(eOuterLoopTeam) == WARPLAN_DOGPILE) && (kTeam.AI_getWarPlanStateCounter(eOuterLoopTeam) < 20)) {
						for (TeamTypes eInnerLoopTeam = (TeamTypes)0; eInnerLoopTeam < MAX_CIV_TEAMS; eInnerLoopTeam = (TeamTypes)(eInnerLoopTeam + 1)) {
							if ((eInnerLoopTeam != eOuterLoopTeam) && eInnerLoopTeam != getTeam() && GET_TEAM(eInnerLoopTeam).isAlive()) {
								if (atWar(eOuterLoopTeam, eInnerLoopTeam) && !kOuterLoopTeam.isAVassal()) {
									iCrushValue += 4;
								}
							}
						}
	...
	{//production
		...
		for (TeamTypes eLoopTeam = (TeamTypes)0; eLoopTeam < MAX_CIV_TEAMS; eLoopTeam = (TeamTypes)(eLoopTeam + 1)) {
			const CvTeam& kLoopTeam = GET_TEAM(eLoopTeam);
			if (kLoopTeam.isAlive() && !kLoopTeam.isMinorCiv()) {
				switch (kTeam.AI_getWarPlan(eLoopTeam)) {
				case NO_WARPLAN:
					break;
...

	{
		...
		for (TeamTypes eOtherTeam = (TeamTypes)0; eOtherTeam < MAX_CIV_TEAMS; eOtherTeam = (TeamTypes)(eOtherTeam + 1)) {
			...
			if (kOtherTeam.isAlive() && eOtherTeam != getTeam()) {
				...
				if (kVictory.isDiploVote()) {
					...
					for (TeamTypes eLoopTeam = (TeamTypes)0; eLoopTeam < MAX_CIV_TEAMS; eLoopTeam = (TeamTypes)(eLoopTeam + 1)) {
						if (eLoopTeam != getTeam()) {
							CvTeamAI& kLoopTeam = GET_TEAM(eLoopTeam);
							if (kLoopTeam.isAlive()) {
								if (GC.getGame().getTeamScore(getTeam()) < ((GC.getGame().getTeamScore(eLoopTeam) * 90) / 100)) {
									iHigherCount++;
									if (kTeam.getPower(true) > kLoopTeam.getPower(true)) {
										IWeakerCount++;
									}
