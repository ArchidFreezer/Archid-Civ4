...
#define PATH_RIVER_WEIGHT       (20) // K-Mod ( * river crossing penalty) - was 100
#define PATH_CITY_WEIGHT        (200) // K-Mod - was 100
#define PATH_DEFENSE_WEIGHT     (4) // K-Mod. ( * defence bonus) - was 10
...
int getCombatOdds(const CvUnit* pAttacker, const CvUnit* pDefender) {
	...
	int iNeededRoundsAttacker = (std::max(0, pDefender->currHitPoints() - iDefenderHitLimit) + iDamageToDefender - 1) / iDamageToDefender;
	...
	int iAttackerLowFS = (pDefender->immuneToFirstStrikes()) ? 0 : pAttacker->firstStrikes();
	int iAttackerHighFS = (pDefender->immuneToFirstStrikes()) ? 0 : (pAttacker->firstStrikes() + pAttacker->chanceFirstStrikes());

	int iDefenderLowFS = (pAttacker->immuneToFirstStrikes()) ? 0 : pDefender->firstStrikes();
	int iDefenderHighFS = (pAttacker->immuneToFirstStrikes()) ? 0 : (pDefender->firstStrikes() + pDefender->chanceFirstStrikes());
	...
	for (int iI = iAttackerLowFS; iI < iAttackerHighFS + 1; iI++) {
		for (int iJ = iDefenderLowFS; iJ < iDefenderHighFS + 1; iJ++) {
			// for every possible combination of fs results, calculate the chance

			if (iI >= iJ) {
				// Attacker gets more or equal first strikes than defender

				int iFirstStrikes = iI - iJ;
				...
				for (int iI3 = 0; iI3 < (iFirstStrikes + 1); iI3++) {
					...
					float fOddsEvent = ((float)getBinomialCoefficient(iFirstStrikes, iI3)) * pow((((float)iAttackerOdds) / GC.getCOMBAT_DIE_SIDES()), iI3) * pow((1.0f - (((float)iAttackerOdds) / GC.getCOMBAT_DIE_SIDES())), (iFirstStrikes - iI3));
			...
			} else // (iI < iJ)
			{
				...
				for (int iI3 = 0; iI3 < (iFirstStrikes + 1); iI3++) {
					...
					if (iI3 < iNeededRoundsDefender) {
						...
						float fOddsEvent = ((float)getBinomialCoefficient(iFirstStrikes, iI3)) * pow((((float)iDefenderOdds) / GC.getCOMBAT_DIE_SIDES()), iI3) * pow((1.0f - (((float)iDefenderOdds) / GC.getCOMBAT_DIE_SIDES())), (iFirstStrikes - iI3));
...
int plotGroupValid(FAStarNode* parent, FAStarNode* node, int data, const void* pointer, FAStar* finder) {
	...
	CvPlot* pOldPlot = GC.getMapINLINE().plotSorenINLINE(parent->m_iX, parent->m_iY);
	CvPlot* pNewPlot = GC.getMapINLINE().plotSorenINLINE(node->m_iX, node->m_iY);
...
int baseYieldToSymbol(int iNumYieldTypes, int iYieldStack) {
	...
	int iReturn = iNumYieldTypes * GC.getDefineINT("MAX_YIELD_STACK");
...
int LFBcalculateCombatOdds(int iFirstStrikes, int iNeededRoundsAttacker, int iNeededRoundsDefender, int iAttackerOdds) {
	...
	if (iFirstStrikes > 0) {
		...
		for (int iI3 = 0; iI3 < (iFirstStrikes + 1); iI3++) {
			...
			float fOddsEvent = ((float)getBinomialCoefficient(iFirstStrikes, iI3)) * std::pow((((float)iAttackerOdds) / GC.getCOMBAT_DIE_SIDES()), iI3) * std::pow((1.0f - (((float)iAttackerOdds) / GC.getCOMBAT_DIE_SIDES())), (iFirstStrikes - iI3));
	...
	} else // (iI < iJ)
	{
		...
		for (int iI3 = 0; iI3 < (iFirstStrikes + 1); iI3++) {
			...
			if (iI3 < iNeededRoundsDefender) {
				...
				float fOddsEvent = ((float)getBinomialCoefficient(iFirstStrikes, iI3)) * std::pow((((float)iDefenderOdds) / GC.getCOMBAT_DIE_SIDES()), iI3) * std::pow((1.0f - (((float)iDefenderOdds) / GC.getCOMBAT_DIE_SIDES())), (iFirstStrikes - iI3));
