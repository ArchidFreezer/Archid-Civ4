...
AreaAITypes CvTeamAI::AI_calculateAreaAIType(CvArea* pArea, bool bPreparingTotal) const {
	...
	if (bTargets) {
		...
		// I'm using AI_countMilitaryWeight; but what I really want is "border territory which needs defending"
...
int CvTeamAI::AI_calculateCapitalProximity(TeamTypes eTeam) const {
	...
	for (int iI = 0; iI < MAX_PLAYERS; iI++) {
		if (GET_PLAYER((PlayerTypes)iI).isAlive()) {
			if (GET_PLAYER((PlayerTypes)iI).getTeam() == getID()) {
				CvCity* pOurCapitalCity = GET_PLAYER((PlayerTypes)iI).getCapitalCity();

				if (pOurCapitalCity != NULL) {
					for (int iJ = 0; iJ < MAX_PLAYERS; iJ++) {
						if (GET_PLAYER((PlayerTypes)iJ).isAlive()) {
							if (GET_PLAYER((PlayerTypes)iJ).getTeam() != getID()) {
								CvCity* pTheirCapitalCity = GET_PLAYER((PlayerTypes)iJ).getCapitalCity();
...
bool CvTeamAI::AI_shareWar(TeamTypes eTeam) const {
	for (TeamTypes eLoopTeam = (TeamTypes)0; eLoopTeam < MAX_CIV_TEAMS; eLoopTeam = (TeamTypes)(eLoopTeam + 1)) {
		const CvTeam& kTeam = GET_TEAM(eLoopTeam);
		if (kTeam.isAlive() && !kTeam.isMinorCiv()) {
			if ((eLoopTeam != getID()) && (eLoopTeam != eTeam)) {
				if (isAtWar(eLoopTeam) && kTeam.isAtWar(eLoopTeam)) {
					return true;
				}
			}
		}
...
AttitudeTypes CvTeamAI::AI_getAttitude(TeamTypes eTeam, bool bForced) const {
	...
	for (PlayerTypes ePlayer1 = (PlayerTypes)0; ePlayer1 < MAX_PLAYERS; ePlayer1 = (PlayerTypes)(ePlayer1 + 1)) {
		if (GET_PLAYER(ePlayer1).isAlive()) {
			if (GET_PLAYER(ePlayer1).getTeam() == getID()) {
				for (PlayerTypes ePlayer2 = (PlayerTypes)0; ePlayer2 < MAX_PLAYERS; ePlayer2 = (PlayerTypes)(ePlayer2 + 1)) {
					if (GET_PLAYER(ePlayer2).isAlive() && ePlayer1 != ePlayer2) {
						TeamTypes eTeamLoop = GET_PLAYER(ePlayer2).getTeam();

						if (eTeamLoop == eTeam) // K-Mod. Removed attitude averaging between vassals and masters
						{
							iAttitude += GET_PLAYER(ePlayer1).AI_getAttitudeVal(ePlayer2, bForced); // bbai. Average values rather than attitudes directly.
							iCount++;
						}
					}
				}
